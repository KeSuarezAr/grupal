...
class State(rx.State):
    ...
    def get_current_month_values(self):
        ...
        current_month_assignments = [
            assignment for assignment in self.assignments if
            datetime.strptime(assignment.date, '%Y-%m-%d %H:%M:%S') >= start_of_month
        ]
        ...
        num_completed = len(
            [assignment for assignment in current_month_assignments if assignment.status == "Completed"])
        self.current_month_values = MonthValues(num_assignments=num_assignments, total_points=total_points,
    ...
    def get_previous_month_values(self):
        ...
        num_completed = len(
            [assignment for assignment in previous_month_assignments if assignment.status == "Completed"]) + 5

        self.previous_month_values = MonthValues(num_assignments=num_assignments, total_points=total_points,
                                                 num_completed=num_completed)
    ...
    def add_assignment_to_db(self, form_data: dict):
        ...
        with rx.session() as session:
            ...
            session.commit()
        self.load_entries()
        return rx.toast.info(f"Assignment {self.current_assignment['title']} has been added.", variant="outline",
    ...
    def update_assignment_to_db(self, form_data: dict):
        ...
        with rx.session() as session:
            ...
            session.commit()
        self.load_entries()
        return rx.toast.info(f"Assignment {self.current_assignment['title']} has been modified.", variant="outline",
                             position="bottom-right")
    ...
    def delete_assignment(self, id: int):
        ...
        with rx.session() as session:
            ...
            session.commit()
        self.load_entries()
        return rx.toast.info(f"Assignment {assignment.title} has been deleted.", variant="outline",
                             position="bottom-right")
    ...
    def assignments_change(self) -> float:
        return _get_percentage_change(self.current_month_values.num_assignments,
                                      self.previous_month_values.num_assignments)
